...
namespace :links do
  ...
  task 'report_outbound': :environment do
    ...
    table = Terminal::Table.new do |t|
      get_links_on_a_page("#{ENV['DOCS_BASE_PATH']}/_documentation/**/*.md").each do |from, to|
  ...
  task 'report_inbound': :environment do
    ...
    filter = ARGV[1]
    table = Terminal::Table.new do |t|
      get_links_to_a_page("#{ENV['DOCS_BASE_PATH']}/_documentation/**/*.md").each do |to, from|
...
def get_all_links(path)
  Dir.glob(path).first(10000).each do |filename|
    ...
    # Get our current URL
    current_page = filename.gsub("#{ENV['DOCS_BASE_PATH']}/_documentation", '').gsub(/\.md$/, '')
