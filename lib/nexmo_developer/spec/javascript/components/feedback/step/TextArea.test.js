...
describe('TextArea', function() {
  const step = { content: 'Some content', type: 'textarea', label: 'Anything we can improve on?', placeholder: 'Hint text' };
  ...
  it('renders its content and corresponding buttons', function() {
    ...
    expect(wrapper.find('label').text()).toEqual(step.label);
  ...
  describe('clicking Continue', function() {
    ...
    it('emits next-step event when textarea is not empty', async function() {
      ...
      expect(eventHub.$emit).toHaveBeenCalledWith('next-step');
