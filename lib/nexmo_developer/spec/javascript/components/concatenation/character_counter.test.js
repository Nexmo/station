...
describe('#isUnicodeRequired', function() {
  ...
  test('when all GSM standard and extended characters are used', function() {
    let string = `@0¡P¿p£_!1AQaq$\"2BRbr¥?#3CScsè?4DTdté?%5EUeuù6FVfvì?'7GWgwò(8HXhxÇ)9IYiy*:JZjzØ+;KÄkäÆ<Llöæ-=MÑmñÅß.>NÜnüåÉ/O§oàΔΦΓΛΩΠΨΣΘΞÖÄÜø¤&|^€{}[]~\\`;
    let counter = new CharacterCounter(string);
    expect(counter.isUnicodeRequired()).toBeFalsy();
  });
  ...
  test('using standard characters', function() {
    let counter = new CharacterCounter('ø Ö Δ Φ Γ Λ Ω Π Ψ Σ Θ Ξ')
    expect(counter.isUnicodeRequired()).toBeFalsy();
  });
});
...
describe('#getMessages', function() {
  ...
  describe('more than 2 SMS', function() {
    ...
    test('3 SMS, considering METADATA size', function() {
      let text = '';
      expect(new CharacterCounter(text).getMessages()).toEqual(
        [
          'start5678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789end',
          'start78x0123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901end',
          'xxxxxx'
        ]
      );
    });
