...
RSpec.describe ImprovePagePresenter do
  ...
  describe '#github_url' do
    it 'returns the url to the doc' do
      expect(subject.github_url).to eq('https://github.com/nexmo/nexmo-developer/blob/main/_documentation/en/concepts/overview.md')
    end
    ...
    context 'when the doc is inside Station' do
      ...
      it 'returns a url to the doc in Station' do
        expect(described_class.new(document_path).github_url).to eq('https://github.com/nexmo/station/blob/main/lib/nexmo_developer/app/views/contribute/overview.md')
      end
    end
  end
