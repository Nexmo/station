module Translator
  class FilesListCoordinator
    ...
    def files
      @files ||= `git log --since="#{@days}".days --name-only --oneline --diff-filter=AMR --pretty=format: #{Rails.configuration.docs_base_path}/_documentation/en #{Rails.configuration.docs_base_path}/_tutorials/en #{Rails.configuration.docs_base_path}/_use_cases/en | uniq | awk 'NF'`.split("\n")
    end
