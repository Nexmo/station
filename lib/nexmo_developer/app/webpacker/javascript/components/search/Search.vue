      ...
      <div class="Nxd-search-input">
        <div class="Vlt-composite Vlt-composite--appendedicon">
          <div class="Vlt-composite__prepend Vlt-composite__prepend--icon">
            <svg>
              <use xlink:href="/symbol/volta-icons.svg#Vlt-icon-search" />
            </svg>
          </div>
          <div class="Vlt-input">
            <input
          ...
          <div
            class="Vlt-composite__append Vlt-composite__append--icon Nxd-search-close"
          >
            <svg>
              <use xlink:href="/symbol/volta-icons.svg#Vlt-icon-cross-bold" />
            </svg>
          </div>
        </div>
      </div>
...
export default {
  ...
  created: function () {
    this.client = algoliasearch(
      document
        .querySelector("meta[name=algolia_application_id]")
        .getAttribute("content"),
      document
        .querySelector("meta[name=algolia_search_key]")
        .getAttribute("content")
    );
  },
  mounted: function () {
    if (document.querySelector(".Nxd-template")) {
      document
        .querySelector(".Nxd-template")
        .addEventListener("click", this.onClickOutside.bind(this));
    } else if (document.querySelector(".Adp-landing")) {
      document
        .querySelector(".Adp-landing .wrapper")
        .addEventListener("click", this.onClickOutside.bind(this));
    }
    this.$el
      .querySelector(".Nxd-search-icon")
      .addEventListener("click", this.handleSearchToggle.bind(this));
    this.$el
      .querySelector(".Nxd-search-close")
      .addEventListener("click", this.handleSearchToggle.bind(this));
  },
  onDestroy: function () {
    if (document.querySelector(".Nxd-template")) {
      document
        .querySelector(".Nxd-template")
        .removeEventListener("click", this.onClickOutside.bind(this));
    } else if (document.querySelector(".Adp-landing")) {
      document
        .querySelector(".Adp-landing .wrapper")
        .removeEventListener("click", this.onClickOutside.bind(this));
  ...
  methods: {
    ...
    handleSearch: function (event) {
      ...
      this.performSearch();
      this.resetAnalyticsListeners();
      this.analyticsStrongIndicationOfReadingTimer = setTimeout(
        this.triggerAnalyticalSearch,
        2000
      );
    ...
    performSearch: function (analytics = false) {
      const parameters = Array.from(
        document.querySelectorAll("meta[name=algolia_index]")
      ).map((element) => {
        return {
