...
To verify the PIN submitted by the user you use the Nexmo library to make a [Verify Check](https://docs.nexmo.com/verify/api-reference/api-reference#check) request. You pass in the `request_id` (which we stored in the user's session data) and the PIN entered by the user.
